...
function get_useragent_results($uas, $row, $browser, $max) {
	...
	foreach ($uas as $ua) {
		...
		$out .= "<div class='$status " . $browser["name"] . ' ' . $browser["os"] . "' style='height: " . $result_size . "em;'><a href='" .
		$GLOBALS['contextpath'] . "/?state=runresults&run_id=" . $row["run_id"] . "&client_id=" . $ua["client_id"] . "'>" .
				($ua["status"] == 2 ?
				($ua["total"] < 0 ?
				"0" :
				($ua["error"] > 0 ?
...
function get_jobstatus_controls($job_id) {

	$jobstatus_controls =
	
	'<a style="border:none" href="'. $GLOBALS['contextpath'] .
...
function get_results_table($browsers, $runs, $job_id) {

	generate_results_table(&$browsers, &$runs, $job_id);

	$results_table = '<div class="result-table"><table class="results"><tbody>';
	$results_table .= generate_browser_header_row($browsers);

	foreach ($runs as $run) {
		$results_table .= "<tr><th nowrap><a href='" . $run["run_url"] . "'>" . $run["run_name"] . "</a></th>\n";
		...
		foreach ($browsers as $browser => $oses) {
			...
			foreach ($oses as $browser) {
				...
				if (array_key_exists($key, $useragents)) {
					...
					$results_table .= get_useragent_results($run_results, $run, $browser, $max);
				} else {
					$results_table .= "<td class='notstarted notdone " . $browser["name"] . " " . $browser["os"] . "'>&nbsp;</td>";
				}
			}
		}
		$results_table .= "</tr>";
