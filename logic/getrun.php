<?php
	...
	$result = mysql_queryf("SELECT run_id FROM run_useragent WHERE useragent_id=%u AND status<>2 AND NOT EXISTS (SELECT 1 FROM run_client WHERE run_useragent.run_id=run_id AND client_id=%u) ORDER BY run_id DESC LIMIT 1;", $useragent_id, $client_id);
